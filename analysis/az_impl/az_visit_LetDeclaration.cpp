    ...
    for (size_t i = 0; i < node.names.size(); ++i) {
        ...
        // --- HANDLE VariableAccess expressions and PROPAGATE class_name ---
            ...
            if (symbol_table_) {
                ...
                if (symbol_table_->lookup(var_access->name, rhs_symbol) && !rhs_symbol.class_name.empty()) {
                    ...
                    if (trace_enabled_) {
                        std::cerr << "[LET DECLARATION VISITOR] Propagated class name '" << determined_class_name 
                                  << "' from '" << var_access->name << "' to '" << name << "'" << std::endl;
                    }
                }
            }
        }
        ...
        if (symbol_table_) {
            ...
            if (!determined_class_name.empty()) {
                ...
                if (trace_enabled_) {
                    std::cerr << "[LET DECLARATION VISITOR] Set class_name '" << determined_class_name << "' for symbol '" << name << "'" << std::endl;
                }
            }
