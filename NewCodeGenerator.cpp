...
void NewCodeGenerator::visit(Neon128BitFPairOpStatement& node) {
    std::cout << "[DEBUG CODEGEN] *** VISITING Neon128BitFPairOpStatement! ***" << std::endl;
    debug_print("Generating optimized 128-bit NEON floating point pair operation");
    ...
    // Calculate byte offset for current pair (each FPAIR is 8 bytes)
    std::string offset_reg = register_manager_.acquire_spillable_temp_reg(*this);
    emit(Encoder::create_lsl_imm(offset_reg, index_reg, 3)); // multiply index by 8
    
    // Load two consecutive FPAIRs (16 bytes total) into 128-bit NEON registers
    std::string neon_left = register_manager_.acquire_q_scratch_reg(*this);
    std::string neon_right = register_manager_.acquire_q_scratch_reg(*this);
    std::string neon_result = register_manager_.acquire_q_scratch_reg(*this);
    ...
    // Perform 128-bit NEON floating point operation based on operation type
    switch (node.operation) {
        case 0: // Add
            emit(Encoder::create_fadd_vector_reg(v_result, v_left, v_right, "4S"));
            debug_print("Generated 128-bit NEON FADD for two FPAIRs (.4S arrangement)");
            break;
        case 1: // Subtract  
            emit(Encoder::enc_create_fsub_vector_reg(v_result, v_left, v_right, "4S"));
            debug_print("Generated 128-bit NEON FSUB for two FPAIRs (.4S arrangement)");
            break;
        case 2: // Multiply
            // For FPAIR multiplication, use .4S arrangement (4x32-bit floats)
            // Each FPAIR is two 32-bit floats, so 128-bit holds 2 FPAIRs = 4 floats
            emit(Encoder::create_fmul_vector_reg(v_result, v_left, v_right, "4S"));
            debug_print("Generated 128-bit NEON FMUL for two FPAIRs (.4S arrangement)");
            debug_print("NOTE: Each FPAIR = 2x32-bit floats, so 128-bit = 2 FPAIRs = 4x32-bit elements");
            break;
        default:
            debug_print("Unsupported 128-bit NEON floating point operation: " + std::to_string(node.operation));
            break;
    }
...
void NewCodeGenerator::visit(Neon128BitQuadOpStatement& node) {
    std::cout << "[DEBUG CODEGEN] *** VISITING Neon128BitQuadOpStatement! ***" << std::endl;
    debug_print("Generating optimized 128-bit NEON quad operation");
    ...
    // Calculate byte offset for current quad (each QUAD is 8 bytes)
    std::string offset_reg = register_manager_.acquire_spillable_temp_reg(*this);
    emit(Encoder::create_lsl_imm(offset_reg, index_reg, 3)); // multiply index by 8
    
    // Load two consecutive QUADs (16 bytes total) into 128-bit NEON registers
    std::string neon_left = register_manager_.acquire_q_scratch_reg(*this);
    std::string neon_right = register_manager_.acquire_q_scratch_reg(*this);
    std::string neon_result = register_manager_.acquire_q_scratch_reg(*this);
    ...
    // Perform 128-bit NEON operation based on operation type
    // Each QUAD is 4×16-bit ints, so 2 QUADs = 8×16-bit ints (.8H arrangement)
    switch (node.operation) {
        case 0: // Add
            emit(Encoder::create_add_vector_reg(v_result, v_left, v_right, "8H"));
            debug_print("Generated 128-bit NEON ADD for two QUADs (.8H arrangement)");
            break;
        case 1: // Subtract  
            emit(Encoder::create_sub_vector_reg(v_result, v_left, v_right, "8H"));
            debug_print("Generated 128-bit NEON SUB for two QUADs (.8H arrangement)");
            break;
        case 2: // Multiply
            emit(Encoder::create_mul_vector_reg(v_result, v_left, v_right, "8H"));
            debug_print("Generated 128-bit NEON MUL for two QUADs (.8H arrangement)");
            debug_print("NOTE: Each QUAD = 4×16-bit ints, so 128-bit = 2 QUADs = 8×16-bit elements");
            break;
        default:
            debug_print("Unsupported 128-bit NEON quad operation: " + std::to_string(node.operation));
            break;
    }
...
void NewCodeGenerator::visit(Neon128BitFQuadOpStatement& node) {
    std::cout << "[DEBUG CODEGEN] *** VISITING Neon128BitFQuadOpStatement! ***" << std::endl;
    debug_print("Generating optimized 128-bit NEON floating point quad operation");
    ...
    // Calculate byte offset for current fquad (each FQUAD is 8 bytes)
    std::string offset_reg = register_manager_.acquire_spillable_temp_reg(*this);
    emit(Encoder::create_lsl_imm(offset_reg, index_reg, 3)); // multiply index by 8
    
    // Load two consecutive FQUADs (16 bytes total) into 128-bit NEON registers
    std::string neon_left = register_manager_.acquire_q_scratch_reg(*this);
    std::string neon_right = register_manager_.acquire_q_scratch_reg(*this);
    std::string neon_result = register_manager_.acquire_q_scratch_reg(*this);
    ...
    // Perform 128-bit NEON floating point operation based on operation type
    // Each FQUAD is 4×16-bit floats, so 2 FQUADs = 8×16-bit floats (.8H arrangement)
    switch (node.operation) {
        case 0: // Add
            emit(Encoder::create_fadd_vector_reg(v_result, v_left, v_right, "8H"));
            debug_print("Generated 128-bit NEON FADD for two FQUADs (.8H arrangement)");
            break;
        case 1: // Subtract  
            emit(Encoder::enc_create_fsub_vector_reg(v_result, v_left, v_right, "8H"));
            debug_print("Generated 128-bit NEON FSUB for two FQUADs (.8H arrangement)");
            break;
        case 2: // Multiply
            emit(Encoder::create_fmul_vector_reg(v_result, v_left, v_right, "8H"));
            debug_print("Generated 128-bit NEON FMUL for two FQUADs (.8H arrangement)");
            debug_print("NOTE: Each FQUAD = 4×16-bit floats, so 128-bit = 2 FQUADs = 8×16-bit elements");
            break;
        default:
            debug_print("Unsupported 128-bit NEON floating point quad operation: " + std::to_string(node.operation));
            break;
    }
...
void NewCodeGenerator::visit(PairwiseReductionLoopStatement& node) {
    ...
    std::cout << "[DEBUG CODEGEN] Processing PAIRWISE_MIN for vectors: " << node.vector_a_name 
              << " and " << node.vector_b_name << std::endl;
    
    // Step 1: Get the type of the input vectors from the symbol table
    ...
    if (!lookup_symbol(node.vector_a_name, vector_a_symbol)) {
        ...
        std::cout << "[DEBUG CODEGEN] ERROR: Symbol not found for " << node.vector_a_name << std::endl;
        return;
    }
    
    debug_print("Vector " + node.vector_a_name + " has type: " + vartype_to_string(vector_a_symbol.type));
    std::cout << "[DEBUG CODEGEN] Vector " << node.vector_a_name << " has type: " 
              << vartype_to_string(vector_a_symbol.type) << std::endl;
    
    // Step 2: Switch on the vector type to generate appropriate NEON instructions
    VarType base_type = vector_a_symbol.type;
    
    // Remove pointer wrapper if present
    if (static_cast<int64_t>(base_type) & static_cast<int64_t>(VarType::POINTER_TO)) {
        std::cout << "[DEBUG CODEGEN] Removing POINTER_TO wrapper from type" << std::endl;
        base_type = static_cast<VarType>(static_cast<int64_t>(base_type) & ~static_cast<int64_t>(VarType::POINTER_TO));
        std::cout << "[DEBUG CODEGEN] Base type after unwrapping: " << vartype_to_string(base_type) << std::endl;
    }
    ...
    if (static_cast<int64_t>(base_type) & static_cast<int64_t>(VarType::FPAIRS)) {
        ...
        if (static_cast<int64_t>(base_type) & static_cast<int64_t>(VarType::VEC)) {
            ...
            std::cout << "[DEBUG CODEGEN] Generating float vector pairwise min (FVEC)" << std::endl;
            generateFloatVectorPairwiseMin(node);
        } else if (base_type == VarType::FPAIR) {
            ...
            debug_print("Unsupported float type for PAIRWISE_MIN: " + vartype_to_string(base_type));
            std::cout << "[DEBUG CODEGEN] ERROR: Unsupported float type: " << vartype_to_string(base_type) << std::endl;
        }
    } else if (static_cast<int64_t>(base_type) & static_cast<int64_t>(VarType::INTEGER)) {
        std::cout << "[DEBUG CODEGEN] Type contains INTEGER, checking subtype..." << std::endl;
        if (static_cast<int64_t>(base_type) & static_cast<int64_t>(VarType::VEC)) {
            // POINTER_TO_INT_VEC: General integer vector (VEC)
        ...
        std::cout << "[DEBUG CODEGEN] ERROR: Completely unsupported type: " << vartype_to_string(base_type) << std::endl;
        return;
    }
    
    debug_print("Completed type-aware NEON pairwise reduction generation");
    std::cout << "[DEBUG CODEGEN] *** PairwiseReductionLoopStatement visit complete! ***" << std::endl;
}

// Helper function for FPAIR (2 floats in 64 bits)
void NewCodeGenerator::generateFPairPairwiseMin(const PairwiseReductionLoopStatement& node) {
    debug_print("Generating FPAIR pairwise minimum (2 floats, 64-bit)");
...
void NewCodeGenerator::generateFOctPairwiseMin(const PairwiseReductionLoopStatement& node) {
    debug_print("Generating FOCT pairwise minimum (8 floats, 128-bit)");
    ...
    // Load 128-bit FOCT values into NEON registers
    emit(Encoder::create_ld1_vector_reg(va_neon, vector_a_reg, "4S")); // 4x32-bit floats
    emit(Encoder::create_ld1_vector_reg(vb_neon, vector_b_reg, "4S")); // 4x32-bit floats
...
// Helper function for general float vectors (FVEC)
void NewCodeGenerator::generateFloatVectorPairwiseMin(const PairwiseReductionLoopStatement& node) {
    debug_print("Generating float vector pairwise minimum (FVEC)");
    ...
    // Load float vectors - assume 4S format for typical FVEC
    emit(Encoder::create_ld1_vector_reg(va_neon, vector_a_reg, "4S"));
    emit(Encoder::create_ld1_vector_reg(vb_neon, vector_b_reg, "4S"));
...
// Helper function for PAIR (2 ints in 64 bits)
void NewCodeGenerator::generatePairPairwiseMin(const PairwiseReductionLoopStatement& node) {
    debug_print("Generating PAIR pairwise minimum (2 ints, 64-bit)");
    ...
    // Load 64-bit PAIR values into NEON registers
    emit(Encoder::create_ldr_vec_imm(va_neon, vector_a_reg, 0)); // Load 64-bit
    emit(Encoder::create_ldr_vec_imm(vb_neon, vector_b_reg, 0)); // Load 64-bit
    
    // SMIN.2S: Element-wise minimum of two 2-element integer vectors
    emit(Encoder::create_smin_vector_reg(vresult_neon, va_neon, vb_neon, "2S"));
...
// Helper function for QUAD (4 ints in 64 bits - actually should be 128 bits)
void NewCodeGenerator::generateQuadPairwiseMin(const PairwiseReductionLoopStatement& node) {
    debug_print("Generating QUAD pairwise minimum (4 ints, 128-bit)");
    ...
    // Load 128-bit QUAD values into NEON registers
    emit(Encoder::create_ld1_vector_reg(va_neon, vector_a_reg, "4S")); // 4x32-bit integers
    emit(Encoder::create_ld1_vector_reg(vb_neon, vector_b_reg, "4S")); // 4x32-bit integers
    
    // SMIN.4S: Element-wise minimum of two 4-element integer vectors
...
// Helper function for OCT (8 ints in 64 bits - should be 256 bits or use multiple registers)
void NewCodeGenerator::generateOctPairwiseMin(const PairwiseReductionLoopStatement& node) {
    debug_print("Generating OCT pairwise minimum (8 ints, requires multiple registers)");
    
    std::string vector_a_reg = get_variable_register(node.vector_a_name);
    std::string vector_b_reg = get_variable_register(node.vector_b_name);
    std::string result_reg = get_variable_register(node.result_vector_name);
    
    // For 8 integers, we need to use two 128-bit registers
    std::string va_neon_lo = register_manager_.acquire_vec_scratch_reg();
    std::string va_neon_hi = register_manager_.acquire_vec_scratch_reg();
    std::string vb_neon_lo = register_manager_.acquire_vec_scratch_reg();
    std::string vb_neon_hi = register_manager_.acquire_vec_scratch_reg();
    std::string vresult_lo = register_manager_.acquire_vec_scratch_reg();
    std::string vresult_hi = register_manager_.acquire_vec_scratch_reg();
    
    // Load lower 4 integers (elements 0-3)
    emit(Encoder::create_ld1_vector_reg(va_neon_lo, vector_a_reg, "4S"));
    emit(Encoder::create_ld1_vector_reg(vb_neon_lo, vector_b_reg, "4S"));
    
    // Load higher 4 integers (elements 4-7) - offset by 16 bytes
    emit(Encoder::create_ldr_imm(va_neon_hi, vector_a_reg, 16));
    emit(Encoder::create_ldr_imm(vb_neon_hi, vector_b_reg, 16));
    
    // SMIN.4S for both halves
    emit(Encoder::create_smin_vector_reg(vresult_lo, va_neon_lo, vb_neon_lo, "4S"));
    emit(Encoder::create_smin_vector_reg(vresult_hi, va_neon_hi, vb_neon_hi, "4S"));
    
    // Store both halves
    emit(Encoder::create_st1_vector_reg(vresult_lo, result_reg, "4S"));
    emit(Encoder::create_str_imm(vresult_hi, result_reg, 16));
    
    register_manager_.release_vec_scratch_reg(va_neon_lo);
    register_manager_.release_vec_scratch_reg(va_neon_hi);
    register_manager_.release_vec_scratch_reg(vb_neon_lo);
    register_manager_.release_vec_scratch_reg(vb_neon_hi);
    register_manager_.release_vec_scratch_reg(vresult_lo);
    register_manager_.release_vec_scratch_reg(vresult_hi);
}

// Helper function for general integer vectors (VEC)
void NewCodeGenerator::generateIntegerVectorPairwiseMin(const PairwiseReductionLoopStatement& node) {
    debug_print("Generating integer vector pairwise minimum (VEC)");
    ...
    // Load integer vectors - assume 4S format for typical VEC
    emit(Encoder::create_ld1_vector_reg(va_neon, vector_a_reg, "4S"));
    emit(Encoder::create_ld1_vector_reg(vb_neon, vector_b_reg, "4S"));
    
    // SMIN.4S: Element-wise minimum of integer vectors
    emit(Encoder::create_smin_vector_reg(vresult_neon, va_neon, vb_neon, "4S"));
    
    // Store integer vector result
    emit(Encoder::create_st1_vector_reg(vresult_neon, result_reg, "4S"));
...
void NewCodeGenerator::visit(NewExpression& node) {
    ...
    // --- DEBUG: Add vtable store verification --- (DISABLED TO FIX SIGILL)
    ...
    // --- DEBUG: Verify object pointer before assignment --- (DISABLED TO FIX SIGILL)
