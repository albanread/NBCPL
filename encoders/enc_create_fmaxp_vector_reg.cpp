...
Instruction Encoder::create_fmaxp_vector_reg(const std::string& vd, const std::string& vn, const std::string& vm, const std::string& arrangement) {
    ...
    if (arrangement != "2S" && arrangement != "4S" && arrangement != "2D") {
        throw std::runtime_error("Invalid arrangement for FMAXP vector: " + arrangement + " (expected 2S, 4S, or 2D)");
    }
    ...
    if (arrangement == "2S") {
        ...
        // 01101110001mmmmm111100nnnnnddddd
        encoding = 0x2E20F000 | (rm << 16) | (rn << 5) | rd;
    } else if (arrangement == "4S") {
        // FMAXP Vd.4S, Vn.4S, Vm.4S (128-bit vector)
        // 01101110101mmmmm111100nnnnnddddd
        encoding = 0x6E20F000 | (rm << 16) | (rn << 5) | rd;
