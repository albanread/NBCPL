...
#
# =============================================================================
# C++ Function Extractor (Standalone Script)
#
# Description:
#   Finds all files matching `encoder*.cpp` in the current directory,
#   extracts each top-level function, and saves it into a separate file
#   in the `encoders/` subdirectory.
#
# Instructions:
#   1. Save this entire block of code to a file (e.g., `split_encoders.sh`).
#   2. Make the script executable by running this command in your terminal:
#       chmod +x split_encoders.sh
...
if [ ${#files[@]} -eq 0 ]; then
  echo "No 'Encoder*.cpp' files found in the current directory. Nothing to do."
  exit 0
fi
...
mkdir -p encoders
echo "Output directory 'encoders/' is ready."
    ...
    gawk -f - "$file" << 'EOF'
        ...
        function save_block() {
            ...
            if (func_name != "") {
                ...
                filename = "encoders/enc_" func_name ".cpp"
...
echo "Check the 'encoders/' directory for results."
